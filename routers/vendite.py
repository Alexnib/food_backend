...
async def registra_vendite_bulk(data: VenditaBulkPayload, auth_data=Depends(get_user_sede)):
    ...
    try:
        ...
        for item in data.items:
            record = {
                "data_vendita": item.data_vendita.isoformat(),
                "quantita": item.quantita,
                "id_sede": auth_data["id_sede"],
                "id_ricetta": item.id_prodotto_menu if item.id_tipo == "finito" else None,
                "id_prodotto_commerciale": item.id_prodotto_menu if item.id_tipo == "commerciale" else None,
            }
            records.append(record)

        res = supabase.table("vendite").insert(records).execute()
        return {"message": f"{len(res.data)} vendite registrate con successo.", "data": res.data}
